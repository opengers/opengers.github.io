...
- 接下来的`src=172.16.202.12 dst=172.16.200.119 sport=10051 dport=54786`地址和端口和前面是相反的，这部分不是数据包中带有的信息，是conntrack填充的信息，代表conntrack希望收到的响应包信息。意思是若后续conntrack跟踪到某个数据包信息与此部分匹配，则此数据包就是此连接的响应数据包，对于无状态的udp也同样合适，只要这里的ip端口匹配。            

conntrack条目依据其连接类型的不同而不同，相比tcp连接，udp和icmp没有连接建立和关闭过程      

有一个用户空间工具conntrack，其提供了对连接跟踪表的增删改查功能，可以用`yum install conntrack-tools`来安装    
...
如果openstack启用安全组功能，则需要安装这个工具，否则会出现类似下面错误                 
...
需要这个工具是因为在对instance进行安全组/port/ip方面的变更后，需要删除连接跟踪表中旧的相关条目，比如instance之前放通80端口，更新安全组之后禁止80口访问，如果不删除那些目的端口为80的conntrack条目，有可能会出现80端口仍能够访问问题                                                                                   

还有，conntrack机制作用只是跟踪并记录通过它的网络连接及其状态，并把信息更新在连接跟踪表，以提供给iptables做状态匹配使用，它并不能修改过滤数据包                              
